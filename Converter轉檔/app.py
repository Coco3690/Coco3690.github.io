...
def copy_video(video_path):
    ...
    try:
        ...
        if file_extension.lower() == '.webm' or file_extension.lower() == '.mkv':
            ...
            # 删除原始的 webm 或 mkv 档案
            print(f"正在删除原始档案: {video_path}")
            os.remove(video_path)
            print(f"已成功删除档案: {video_path}")
            print(f"已成功将 {video_name} 转换为 {mp4_name} 并删除原档案。")
            return f"已成功将 {video_name} 转换为 {mp4_name} 并删除原档案。"
...
def convert_video(video_path):
    ...
    try:
        ...
        subprocess.run(['ffmpeg', '-y', '-i', video_path, mp3_name], check=True)
        # 使用 ffmpeg 转换 MP4 到 WAV，并自动覆盖
        subprocess.run(['ffmpeg', '-y', '-i', video_path, wav_name], check=True)
...
def split_video(video_path):
    ...
    try:
        shutil.copy(video_path, split_video_path)
